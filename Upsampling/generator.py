...
from tf_lib.gcn_lib.edge import dil_knn
from tf_lib.gcn_lib.vertex import edge_conv
...
class MPU(object):
    ...
    def __call__(self, inputs):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            # coord = edge_conv(coord, 3, idx=idx4, n_layers=1, scope='edge_conv3')
            outputs = tf.squeeze(coord, [2])
...
class PUNET(object):
    ...
    def __call__(self, inputs, bradius):
        ...
        with tf.variable_scope(self.name, reuse=self.reuse):
            ...
            coord += tf.reshape(tf.tile(tf.expand_dims(inputs, 2), [1, 1, self.up_ratio, 1]),
                       [inputs.shape[0], self.num_point * self.up_ratio, -1])  # B, N, 4, 3
