...
def nodeshuffle(x, scale=2, k=20, d=1, channels=64, idx=None,scope='nodeshuffle', **kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels, 1,padding='VALID', scope='up_reduction', **kwargs)
        y = edge_conv(y,channels * scale,idx,n_layers=1,scope='edge_conv',**kwargs)
        y = point_shuffler(y, scale)
        return y

# 原始的分级上采样
def nodeshuffle2(x, scale=2,  k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
    # pro layer with none idx
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
        y = edge_conv(y, channels * 2, idx, k=k, n_layers=1, scope='edge_conv', **kwargs)
        y = point_shuffler(y, 2)
        y = edge_conv(y, channels * 2, idx=None, k=k, n_layers=1, scope='edge_conv2', **kwargs)
        y = point_shuffler(y, 2)
        return y

# 直接在shuffle后做特征级的边缘卷积
def nodeshuffle3(x, scale=2, k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
        y = edge_conv(y, channels * scale, idx, n_layers=1, scope='edge_conv', **kwargs)
        y = point_shuffler(y, scale)
        y = edge_conv(y, channels, idx=None, k=k, n_layers=1, scope='edge_conv1', **kwargs)
        return y

    
# pro version
def pronodeshuffle(x, scale=2, k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
        y = edge_conv(y, channels * 2, idx, n_layers=1, scope='edge_conv', **kwargs)
        y = point_shuffler(y, 2)
        idx2 = tf.tile(idx, [1, 2, 1, 1])
        y = edge_conv(y, channels * 2, idx2, k=k, n_layers=1, scope='edge_conv2', **kwargs)
        y = point_shuffler(y, 2)
        idx4 = tf.tile(idx, [1, 4, 1, 1])
        y = edge_conv(y, channels, idx4, k=k, n_layers=1, scope='edge_conv3', **kwargs)
        return y

# 直接四倍上采样
def pronodeshuffle2(x, scale=2, k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
        y = edge_conv(y, channels * scale, idx, n_layers=1, scope='edge_conv', **kwargs)
        y = point_shuffler(y, scale)
...
# pro 逐级上采样 不用高维边缘卷积
def pronodeshuffle3(x, scale=2, k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
        y = edge_conv(y, channels * 2, idx, n_layers=1, scope='edge_conv', **kwargs)
        y = point_shuffler(y, 2)
...
#         y = edge_conv(y, channels * 2, idx2, k=k, n_layers=1, scope='edge_conv2', **kwargs)
#
#         grid_raw = gen_grid1(2)  # (4, 2)
#         grid = tf.tile(tf.expand_dims(grid_raw, 0), [batch_size, n_points, 1])
#         grid = tf.expand_dims(grid, axis=-2)
#         y = tf.concat([y, grid], axis=-1)
#         y = conv2d(y, channels * 2, [1, 1],
#                    padding='VALID', stride=[1, 1],
#                    scope='up_layer1',
#                    **kwargs)
#
#         _, n_points, _, _ = y.get_shape().as_list()
#         y = point_shuffler(y, 2)
#         idx4 = tf.tile(idx, [1, 4, 1, 1])
#         y = edge_conv(y, channels, idx4, k=k, n_layers=1, scope='edge_conv3', **kwargs)
#
...
# 原始的版本直接四倍上采样 且卷积时无id
# def nodeshuffle(x, scale=2,  k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
#     # original idx is none
#     with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
#         y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
#         y = edge_conv(y, channels * scale, idx=None, k=k, n_layers=1, scope='edge_conv', **kwargs)
#         y = point_shuffler(y, scale)
#         return y

# 分级的特征proshuffle 第一层用proshuffle
# def nodeshuffle(x, scale=2,  k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
#     # pro layer with none idx
#     with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
#         y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
...
# 分级的特征proshuffle 第一层也没有id
# def nodeshuffle(x, scale=2,  k=20, d=1, channels=64, idx=None, scope='nodeshuffle', **kwargs):
#     # all layers with none idx
#     with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
#         y = conv2d(x, channels, 1, padding='VALID', scope='up_reduction', **kwargs)
#         y = edge_conv(y, channels * 2, idx=None, k=k, n_layers=1, scope='edge_conv', **kwargs)
#         y = point_shuffler(y, 2)
#         y = edge_conv(y, channels * 2, idx=None, k=k, n_layers=1, scope='edge_conv2', **kwargs)
#         y = point_shuffler(y, 2)
#         return y
...
def mlpshuffle(x, scale=2, channels=64, scope='mlpshuffle', **kwargs):
    # single mlp
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        y = conv2d(x, channels * scale, 1, padding='VALID', scope='up_reduction', **kwargs)
        y = point_shuffler(y, scale)
        return y
...
#     # promlp
#     with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
#         y = conv2d(x, channels * 2, 1, padding='VALID', scope='up_reduction1', **kwargs)
#         y = conv2d(y, channels * 2, 1, padding='VALID', scope='conv1', **kwargs)
#         y = point_shuffler(y, 2)
#         y = conv2d(y, channels * 2, 1, padding='VALID', scope='up_reduction2', **kwargs)
#         y = point_shuffler(y, 2)
#         return y
...
def gen_grid1(num_grid_point):
    """
    generate unique indicator for duplication based upsampling module.
    output [num_grid_point, 2]
    """
    x = tf.lin_space(-1.0, 1.0, num_grid_point)
    grid = tf.stack([x], axis=-1)
    return grid
...
def duplicate(x, scale, scope='duplicate',
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        grid = gen_grid(np.round(np.sqrt(scale)).astype(np.int32))      # (4, 2)
